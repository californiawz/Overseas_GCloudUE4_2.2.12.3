...
int TGPAServiceiOS::CheckHapticPlayerSupport(HapticPlayerType playerType) {
    int status = TGPA_HapticSupport();
    if (playerType == IOSPatternPlayer) {
        if (status == 1) {
            return 1;
        }
        else if (status == -1) {
...
int TGPAServiceiOS::CheckHapticAmplitudeSupport() {
    return TGPA_HapticAmplitudeSupport();
}

void TGPAServiceiOS::SetCurrentHapticPlayerType(HapticPlayerType playerType) {
    // only one player support
    // ignore this in iOS platform
}

void TGPAServiceiOS::HapticPlay(const FString& heJson, int loop, int interval, int amplitude) {
    TGPA_HapticPlay(TCHAR_TO_UTF8(*heJson), loop, interval, amplitude);
}

void TGPAServiceiOS::HapticPlayWithFile(const FString& heFilePath, int loop, int interval, int amplitude) {
    TGPA_HapticPlayWithFile(TCHAR_TO_UTF8(*heFilePath), loop, interval, amplitude);
}

void TGPAServiceiOS::HapticStop() {
    TGPA_HapticStop();
}