...
enum HapticSupportStatus
{
    InBlackList = -1,              // 不支持振动，当前设备已加入TGPA后台黑名单
    NotSupport = 0,                // 不支持振动，当前设备/系统无法支持
    Support = 1                    // 支持振动
};

enum HapticPlayerType
{
    
    StandardHapticPlayer = 1, IOSPatternPlayer = 1,       // Android平台支持的高品质播放器, IOS平台支持的高品质播放器
    RichTapPlayer = 2,                                    // AAC Player, 已废弃, 请勿使用
    AndroidPatternPlayer = 3                              // Android平台支持的标准品质播放器, iOS平台没有此分类
};
...
public:
    ...
    // 当前设备Haptic支持类型
    virtual int CheckHapticPlayerSupport(HapticPlayerType playerType) = 0;

    // 当前设备是否支持对HE定义的振动波形进行缩放
    virtual int CheckHapticAmplitudeSupport() = 0;

    // 设置当前设备使用的Haptic播放器，设置前请务必确认对应播放器是否支持
    virtual void SetCurrentHapticPlayerType(HapticPlayerType playerType) = 0;

    virtual void HapticPlay(const FString& heJson, int loop, int interval, int amplitude) = 0;

    virtual void HapticPlayWithFile(const FString& heFilePath, int loop, int interval, int amplitude) = 0;
